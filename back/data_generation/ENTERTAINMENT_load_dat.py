""" Script to get Entertainment data from API google Places """
...
url = f'https://maps.googleapis.com/maps/api/place/textsearch/json?query=sitios+de+entrenimiento+familiar+en+Montevideo+Uruguay&language=es-419&key={API_KEY}' # url google API with the search of entertainment palces in Montevideo
...
while True: # loop to get more results until next_page is not None
    for place in places: 
        types = place.get('types') # Get the type list of the place
        # categoriues to be excluded
        ...
        place_id = place.get('place_id') # ID to get further information of the place
        url = f"https://maps.googleapis.com/maps/api/place/details/json?place_id={place_id}&key={API_KEY}" # url google API with the details of a entertainment place
        response = requests.get(url)
        ...
        driver = webdriver.Chrome('/home/vagrant/PlanItNow/back/data_generation/chromedriver', options=chrome_options) # Start the browser with the options previously set and the chrome driver
    ...
    next_page = results.get('next_page_token') # token used to get 20 more results
print(f'The entertainment script was exectued at {datetime.datetime.now().strftime("%d-%m-%Y %H:%M")}')
