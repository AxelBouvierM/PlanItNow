""" Script to get breweries data from API google Places """
...
while True:
    for place in places:
        ...
        url = f'https://maps.googleapis.com/maps/api/place/details/json?place_id={place_id}&key={API_KEY}' # url google API with the details of a brewery
        response = requests.get(url)
        ...
        insert = """INSERT INTO brewery (breweryID, title, image, link, place, date, price, description) VALUES (NULL, %s, %s, %s, %s, %s, %s, %s)"""
    ...
    next_page = results.get('next_page_token') # token used to get 20 more results
print(f'The brewery script was exectued at {datetime.datetime.now().strftime("%d-%m-%Y %H:%M")}')
