""" Script to get museums data from API google Places """
...
url = f'https://maps.googleapis.com/maps/api/place/textsearch/json?query=Museos%20en%20Montevideo%20Uruguay&language=es-419&key={API_KEY}'  # url google API with the search of museums in Montevideo
...
while True:  # loop to get more results until next_page is not None
    for place in places:
        ...
        url = f'https://maps.googleapis.com/maps/api/place/details/json?place_id={place_id}&language=es-419&key={API_KEY}'  # url google API with the details of a museum
        response = requests.get(url)
    ...
    next_page = results.get('next_page_token')  # token used to get 20 more results
print(f'The museum script was exectued at {datetime.datetime.now().strftime("%d-%m-%Y %H:%M")}')
